...
public class MasterHuntScript : MonoBehaviour {
  public int manualRoomCount = 5, manualSeed = -1;
...
[HideInInspector]
    ...
    while (roomCount < maxRooms) {
      ...
      for(int i = 0; i < adjacentRoomCount; i++) {
        ...
        if(roomCount == maxRooms - 1) {
          connectedRoom = createRoom(bossRoomLayouts[random.Next(0, 
            bossRoomLayouts.Count)],newPosition);
        } else {
          connectedRoom = createRoom(nonBossRoomLayouts[random.Next(0, 
            nonBossRoomLayouts.Count)], newPosition);
        }
  ...
  public void createWall(Room givenRoom, Vector2 entrance, bool open) {
    ...
    if(open) {
      givenRoom.transform.Find(position+"Closed").gameObject.SetActive(false);
      givenRoom.transform.Find(position+"OpenA").gameObject.SetActive(true);
      givenRoom.transform.Find(position+"OpenB").gameObject.SetActive(true);
    }
  }
  ...
  public int generateConnectionCount(int entranceCount, 
    Vector3 spawnProbability) {
    int randomValue = random.Next(0,100);
    if(randomValue < spawnProbability.x || entranceCount < 2) {
      return 1;
    } else if(randomValue < spawnProbability.x  + spawnProbability.y || 
      entranceCount < 3) {
      return 2;
    } else if(randomValue < spawnProbability.x  + spawnProbability.y + 
      spawnProbability.z || entranceCount < 4) {
      return 3;
    } else {
      return 4;
    }
